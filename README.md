...
* `expires_in:` - mutex TTL in second (or ActiveSupport::Numeric time interval), lock will be
  removed by redis automatically when expired, lock will expire in 1 hour if not provided
* `signature:` - string used to determine ownership of lock, checked when manually deleting lock,
  optional, will be generated by `SecureRandom.uuid` if not provided
* `payload:` - any object that can be serialized as JSON, optional, will stay empty if not provided
